...
export async function unlisting(params: {
  tag_id: number;
  listing_ids: number[];
}) {
...
export async function quantityOffset(params: {
  listing_id: number;
...
export async function priceOffset(params: {
  listing_id: number;
  price_offset: number;
}) {
...
export async function batchChangeQuantity(params: {
  listing_ids: number[];
  quantity: number;
}) {
