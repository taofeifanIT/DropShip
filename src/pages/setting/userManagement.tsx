...
const OperationModal = (props: { isModalVisible: boolean, setIsModalVisible: (fn: () => void) => void, dataInit: () => void,roleData: role[], record?: any}) => {
  const layout = {
    labelCol: { span: 6 },
    wrapperCol: { span: 13 },
  };
  ...
  const onFinish = () => {
    formRef.current.validateFields().then((value: any) => {
      ...
      api(param).then(res => {
        ...
        form.resetFields()
        props.setIsModalVisible(() => {form.resetFields()})
          props.dataInit()
        } else {
          throw res.message
        }
      ...
      <Modal maskClosable={false} 
        ...
        >
          ...
          {!record ? (<Form.Item
            label="password"
            name="password"
            rules={[{ required: true, validator: async (_, password) => {
              if (!password) {
          ...
          <Form.Item 
            label="tag permission"
            name="open_tag_permission"
...
export default () => {
  ...
  const columns: ProColumns<GithubIssueItem>[] = [
    ...
    },
    {
      ...
      dataIndex: 'id',
      render: (text, row: any) => [
        <a key="show" target="_blank" rel="noopener noreferrer" onClick={() => {
          setRecord(row)
  ...
  return (
    <div style={{height: `${(initialState as any).pageHeight}px` }}>
      <ProTable<GithubIssueItem> actionRef={ref}
        columns={columns}
        ...
        search={false}
        toolBarRender={() => [
          <Button key="userBtn" type="primary" onClick={() => {
            setIsModalVisible(true)
          }}>
            <PlusOutlined />
              Add
      ...
      />
      <OperationModal isModalVisible={isModalVisible} setIsModalVisible={cancelModal} dataInit={dataInit} roleData={roleData} record={record}></OperationModal>
    </div>
  );
};
