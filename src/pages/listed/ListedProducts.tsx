...
const BatchPriceModal = (props: {
  ...
  const ViewHistoryData = (props: {
    ...
    return (<>
        <Button icon={<BarChartOutlined />} style={{...style}} onClick={() => {
                    handleOpenView()
                }}>history data</Button>
         <Modal
      ...
      return (<>
        {
            editStatus ? (
                <>
                    <Input ref={inputEl} size={'small'} style={{width: '120px'}} suffix={<EnterOutlined />} value={editAsin} onPressEnter={(e: any) => {
                ...
                <>
                    <Link href={`${getAsonHref(country_id)}${asin}`} target="_blank">{editAsin}</Link>
                    <EditOutlined  style={{color: '#1890ff', cursor: 'pointer', marginLeft: '6px'}} onClick={() => {
                        setEditStates(true)
                    }} />
                </>
            )
  ...
  const columns = (refresh: () => void, editFn: (visible: boolean, id: number, record: any) => void): ProColumns<GithubIssueItem>[] => [
    ...
    },
    {
        ...
        render: (_, record: any) => {
            ...
            return (
                <>
                    <Space direction="vertical">
                        <Text type="secondary">Marketplace: <Text>{getMarketPlace()}$</Text></Text>
                        <Text type="secondary">After algorithm: <Text>{record.after_algorithm_price}$</Text></Text>
                        <Text type="secondary">Price offset: <Text>{record.price_offset}$</Text></Text>
                        <Text type="secondary">Lowest Listed: <Text>{getText('LandedPrice')}</Text></Text>
                        <Text type="secondary">Buy Box: <Text>{getText('ListingPrice')}</Text></Text>
                    </Space>
                </>
            )
        }
    },
    {
        ...
        render: (_, record: any) => {
            return (
                <>
                    <Space direction="vertical">
                        <Text type="secondary">Marketplace Inventory: <Text>{record.listing_to_store_quantity}</Text></Text>
                        <Text type="secondary">Quantity Offset: <Text>{record.quantity_offset}</Text></Text>
                        <Text type="secondary">vendor quantity: <Text>{record.vendor_quantity}</Text></Text>
                    </Space>
                </>
            )
        }
    },
    {
        ...
        render: (_, record: any) => {
            const getCountryImg = (countryName: string) => {
                switch(countryName){
                    case 'UK':
                        return <img width='20' src={'https://dss2.baidu.com/6ONYsjip0QIZ8tyhnq/it/u=1640819749,3704395080&fm=58&app=83&f=JPG?w=200&h=132&s=7097A97266B303A3091E6AEC0300A006'} />
                    case 'US':
                        return <img width='20' src={'https://dss2.baidu.com/6ONYsjip0QIZ8tyhnq/it/u=806796136,1072976903&fm=58&app=83&f=JPEG?w=200&h=132&s=7063B1546F9C31EBB6AD4FDD03001006'} />
                }
            }
            return (
                <>
                    <Space direction="vertical">
                        <Text type="secondary">Marketplace: <Text>{getKesValue('marketPlaceData',record.marketplace_id)?.marketplace}</Text></Text>
                        <Text type="secondary">Store: <Text>{getKesValue("storeData",record.store_id)?.name}</Text></Text>
                        <Text type="secondary">Country: <Text>{getCountryImg(getKesValue("countryData",record.country_id)?.country)}</Text></Text>
                    </Space>
                </>
            )
        }
    },
    {
        ...
        render: (_, record: any) => {
            return (
                <>
                    <Space direction="vertical">
                        <Text type="secondary">task_update_at: <Text>{(record.task_update_at && moment(parseInt(record.task_update_at + '000')).format("YYYY-MM-DD HH:mm:ss")) || 'not yet'}</Text></Text>
                        <Text type="secondary">update_at: <Text><a onClick={() => {
                            history.push(`/log/OperationLog?batch_id=${record.batch_id}`)
                        }}>{(record.update_at && moment(parseInt(record.update_at + '000')).format("YYYY-MM-DD HH:mm:ss")) || 'not yet'}</a></Text></Text>
    ...
    },
    {
        ...
        render: (_, record: any) => {
            return (
                <>
                    <Space direction="vertical">
                        <Text type="secondary">vendor: <Text>{getKesValue("vendorData",record.vendor_id)?.vendor_name}</Text></Text>
                        <Text type="secondary">vendor_sku: <Text>{record.vendor_sku}</Text></Text>
                        <Text type="secondary">vendor_quantity: <Text>{record.vendor_quantity}</Text></Text>
    ...
    // scroll={{
    {
        ...
        width: 150,
        request: async () => {
            return [...getKesGroup('priceAlgorithmsData').map((item:priceAlgorithms) => {
                return {
                    label: item.name,
                    value: item.id,
                }
    ...
    },
    {
        ...
        hideInTable: true,
        request: async () => {
            return [...getKesGroup('marketPlaceData').map((item:marketplaces) => {
                return {
    ...
    },
    {
        ...
        render: (text, record: {
            ...
            return (
                <>
                   <Button style={{width: '115px'}} size="small" type='primary' onClick={() => {
                       editFn(true, record.id, record)
                   }}><EditOutlined />Edit</Button>
                   <Button loading={loadingBtn} type='ghost' style={{width: '115px', marginTop: '8px'}} disabled={record.listing_status === 'Listed'}  size="small" onClick={() => {
                       relistFn()
                   }}>
                    <RetweetOutlined />Relisting
                   </Button>
                   <ViewHistoryData style={{width: '115px', marginTop: '8px'}} vendor_id={record.vendor_id} vendor_sku={record.vendor_sku}  />
                </>
            )
        }
    },
...
export default () => {
    ...
    return (
        <>
            <Alert showIcon style={{marginBottom: '10px'}} message={
                            `Next Amazon Listing update time:${moment(parseInt(initialState?.listTimes?.getAmazonListingDeliverTime + '000')).format('YYYY-MM-DD HH:mm:ss')}/
                             Next Amazon Feed/update price/quantity update time:${moment(parseInt(initialState?.listTimes?.getAmazonNormalDeliverTime + '000')).format('YYYY-MM-DD HH:mm:ss')}`} />
            ...
            <ProTable<GithubIssueItem>
                ...
                scroll={{ x: columns(refresh, editFn).reduce((sum, e) => sum + Number(e.width || 0), 0), y: getPageHeight() -290 }}
            ...
            <BatchPriceModal batchPriceModalVisible={visible} setBatchPriceModalVisible={setVisible} record={currentItem} listingId={listId} refresh={refresh} />
        </>
    );
};