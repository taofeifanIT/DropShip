...
import { Form, Input,Button, Pagination, Spin, Upload  } from 'antd';
  ...
  return (<>
    ...
    {dataObj.data.map((items: itemPop, index) => {
         return ( <List
          ...
          footer={<div style={{textAlign: 'right'}}><Text type='secondary'>毛利率：<Text style={{fontSize: '21px'}}>{items.gross_margin}</Text></Text></div>}
          ...
          renderItem={item => (
            <List.Item key={'index5' + index}>
              ...
              <div style={{display: 'inline-block'}} key={'content' + index}>
                <Text type='secondary'>
                  Asin: <a
                    target="_blank"
                    rel="noreferrer"
                ...
                <Text type='secondary' ellipsis style={{ width: '150px' }}>
                  描述: <Text>{items.description}</Text>
                </Text>
              </div>
              <Divider type='vertical' style={{height: '70px'}} />
              ...
              <div style={{display: 'inline-block'}} key={'index3' + index}>
                <Text type='secondary'>
                  参考价格: <Text>{items.reference_price}</Text>
                </Text><br />
              </div>
            </List.Item>
          )}
